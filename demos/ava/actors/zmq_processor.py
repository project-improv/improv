...
class AudioProcessor(Actor):
    ...
    def __init__(self, *args, model_path=None, fs=None, params=None, gpu=None, gpu_num=None, time_opt=None, timing=None, out_path=None, method='fork', **kwargs):
    ...
    def stop(self):
        ...
        if self.time_opt is True:
            ...
            df.to_csv(os.path.join(self.timing_path, 'proc_timing' + '.csv'), index=False, header=True)
    ...
    def runProcessor(self):
        ...
        if ids is not None:
            ...
            try:
                ...
                #     seg = seg + str('_skip')
                t3 = time.time()
                spec = self._genSpec(0, self.win_len, audio, self.fs, self.params)
                t4 = time.time()
                ...
                if self.save_spec:
                    ...
                    np.save(os.path.join(spec_path + '.npy'), spec)
                t5 = time.time()
                ...
                if self.save_latent:
                    ...
                    np.save(os.path.join(latent_path + '.npy'), latents)
                t9 = time.time()
