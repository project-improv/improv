...
class PyTorchProcessor(Actor):
    ...
    def runProcess(self):
        ''' Run process. Runs once per image.
            ...
            place the processed image, model output, and classification/predictionwith ref number that
        ...
        if img is not None:
            ...
            self.done = False
            try:
                img = self.client.getID([str(self.img_num)])
                t1 = time.time()
                img = self._processImage(img, self.img_num)
                t2 = time.time()
                output = self._runInference(img)
                t3 = time.time()
                out_obj_id = self.client.put(output.numpy(), 'output' + str(self.img_num))
                # self.put([[out_obj_id, str(self.img_num)]], save=True)
                t4 = time.time()
                if self.classify is True:
